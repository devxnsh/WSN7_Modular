    ...
    methods (Static)
        ...
        function [nodes, posArray] = getStructTopology(N, field)
            ...
            if numel(demotable) < (throwGWNs - targetGWNs)
                ...
                % Normalize to row vectors before concatenating - setdiff/
                % unique orientation depends on input shapes and was
                % observed to mismatch (row vs column) for some random
                % topologies, throwing "Dimensions of arrays being
