...
classdef WSN_Gateway_Messaging < handle
    ...
    methods
        ...
        function mergeSensorAgg(obj, msg, t)
            ...
            % Payload format (matches CH's createSensorAgg in
            ...
            if msg.isEncrypted()
                ...
                payload = WSN_Crypto.decrypt(payload, localKey);
            end
            ...
            numSensors = payload(3);
            offset = 4;  % Start after 3-byte fragment header

            for i = 1:numSensors
                if offset + 7 > numel(payload) + 1
                    break;  % Incomplete entry
                end

                sensorID = typecast(payload(offset:offset+1), 'uint16');
                sensorTime = typecast(payload(offset+2:offset+3), 'uint16');
                sensorValue = typecast(payload(offset+4:offset+5), 'uint16');
                sensorRSSI = double(payload(offset+6)) / 10;
                sensorBattery = double(payload(offset+7));
                offset = offset + 8;
